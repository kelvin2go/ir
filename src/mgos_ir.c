...
static void irrecv_sam_handler(int pin, void *arg)
{
  struct mgos_irrecv_nec_s *obj = (struct mgos_irrecv_nec_s *)arg;
  // get microseconds
  uint32_t t = 1000000 * mgos_uptime();
...
struct mgos_irrecv_nec_s *mgos_irrecv_sam_create(int pin, void (*handler)(int, void *), void *user_data)
{
  ...
  if (
    !mgos_gpio_set_mode(pin, MGOS_GPIO_MODE_INPUT) ||
    !mgos_gpio_set_pull(pin, MGOS_GPIO_PULL_UP) ||
    !mgos_gpio_set_int_handler_isr(pin, MGOS_GPIO_INT_EDGE_ANY, irrecv_sam_handler, (void *)obj) ||
    !mgos_gpio_enable_int(pin)
  ) {
    mgos_irrecv_sam_close(obj);
    return NULL;
  }
  return obj;
}
...
static void irsend_sam_tsop(int pin, int code)
{
  // setup

  // header
  mgos_gpio_set_mode(pin, MGOS_GPIO_MODE_OUTPUT);
  mgos_usleep(SAMSUNG_HDR_MARK);
  mgos_gpio_set_mode(pin, MGOS_GPIO_MODE_INPUT);
  mgos_usleep(SAMSUNG_HDR_SPACE);

  // sequence
  for (uint32_t i = 0x80000000; i; i >>= 1) {
    mgos_gpio_set_mode(pin, MGOS_GPIO_MODE_OUTPUT);
    mgos_usleep(SAMSUNG_BIT_MARK);
    mgos_gpio_set_mode(pin, MGOS_GPIO_MODE_INPUT);
    if (code & i) {
      mgos_usleep(SAMSUNG_ONE_SPACE);
    } else {
      mgos_usleep(SAMSUNG_ZERO_SPACE);
    }
  }
...
static void irsend_sam_pwm(int pin, int code)
{
  ...
  // header
  irsend_carrier_38kHz(pin, SAMSUNG_HDR_MARK / IRSEND_NEC_PWM_CYCLE);
  mgos_usleep(SAMSUNG_HDR_SPACE);

  // sequence
  for (uint32_t i = 0x80000000; i; i >>= 1) {
    irsend_carrier_38kHz(pin, SAMSUNG_BIT_MARK / IRSEND_NEC_PWM_CYCLE);
    if (code & i) {
      mgos_usleep(SAMSUNG_ONE_SPACE);
    } else {
      mgos_usleep(SAMSUNG_ZERO_SPACE);
...
void mgos_irsend_sam(int pin, int code, bool tsop)
{
  ...
  if (tsop) {
    irsend_sam_tsop(pin, code);
  } else {
    irsend_sam_pwm(pin, code);
  }
}
